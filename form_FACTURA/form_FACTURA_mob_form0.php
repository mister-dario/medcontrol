...
.sc-button-image.disabled img {
 ...
 <?php
 if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['embutida_pdf']))
 {
 ?>
...
?>
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>form_FACTURA/form_FACTURA_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
...
<?php
include_once("form_FACTURA_mob_sajax_js.php");
?>
<script type="text/javascript">
...
include_once('form_FACTURA_mob_jquery.php');
...
<?php
$str_iframe_body = ('F' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['run_iframe'] || 'R' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['run_iframe']) ? 'margin: 2px;' : '';
...
if ($GLOBALS["erro_incl"] == 1)
{
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['opc_ant'] = "novo";
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['recarga'] = "novo";
}
if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['recarga']))
{
    $opcao_botoes = $this->nmgp_opcao;
}
else
{
    $opcao_botoes = $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['recarga'];
}
    $remove_margin = '';
    $remove_border = '';
    $vertical_center = '';
...
if (isset($_SESSION['scriptcase']['form_FACTURA']['error_buffer']) && '' != $_SESSION['scriptcase']['form_FACTURA']['error_buffer'])
{
    echo $_SESSION['scriptcase']['form_FACTURA']['error_buffer'];
}
...
<?php
 include_once("form_FACTURA_mob_js0.php");
?>
<script type="text/javascript"> 
...
<form  name="F1" method="post" 
               action="form_FACTURA_mob.php" 
               target="_self">
<input type="hidden" name="nmgp_url_saida" value="">
<?php
if ('novo' == $this->nmgp_opcao || 'incluir' == $this->nmgp_opcao)
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['insert_validation'] = md5(time() . rand(1, 99999));
?>
<input type="hidden" name="nmgp_ins_valid" value="<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['insert_validation']; ?>">
<?php
}
?>
...
<?php
$_SESSION['scriptcase']['error_span_title']['form_FACTURA_mob'] = $this->Ini->Error_icon_span;
$_SESSION['scriptcase']['error_icon_title']['form_FACTURA_mob'] = '' != $this->Ini->Err_ico_title ? $this->Ini->path_icones . '/' . $this->Ini->Err_ico_title : '';
?>
...
<tr><td>
<?php
if (($this->Embutida_form || !$this->Embutida_call || $this->Grid_editavel || $this->Embutida_multi || ($this->Embutida_call && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['embutida_liga_form_btn_nav'])) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['run_iframe'] != "F" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['run_iframe'] != "R")
{
?>
...
<?php
    ...
    if ((!$this->Embutida_call) && ((!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['dashboard_info']['under_dashboard']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['dashboard_info']['under_dashboard'] || (isset($this->is_calendar_app) && $this->is_calendar_app)))) {
        $sCondStyle = (isset($_SESSION['scriptcase']['nm_sc_retorno']) && !empty($_SESSION['scriptcase']['nm_sc_retorno']) && $nm_apl_dependente != 1 && $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['run_iframe'] != "F" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['run_iframe'] != "R" && !$this->aba_iframe && $this->nmgp_botoes['exit'] == "on") ? '' : 'display: none;';
?>
...
<?php
    ...
    if ((!$this->Embutida_call) && ((!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['dashboard_info']['under_dashboard']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['dashboard_info']['under_dashboard'] || (isset($this->is_calendar_app) && $this->is_calendar_app)))) {
        $sCondStyle = (!isset($_SESSION['scriptcase']['nm_sc_retorno']) || empty($_SESSION['scriptcase']['nm_sc_retorno']) || $nm_apl_dependente == 1 || $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['run_iframe'] == "F" || $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['run_iframe'] == "R" || $this->aba_iframe || $this->nmgp_botoes['exit'] != "on") && ($_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['run_iframe'] != "R" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['run_iframe'] != "F" && $this->nmgp_botoes['exit'] == "on") && ($nm_apl_dependente == 1 && $this->nmgp_botoes['exit'] == "on") ? '' : 'display: none;';
?>
...
<?php if ($sc_hidden_no > 0) { echo "<tr>"; }; 
      $sc_hidden_yes = 0; $sc_hidden_no = 0; ?>
<?php
           if ('novo' != $this->nmgp_opcao && !isset($this->nmgp_cmp_readonly['num_cita']))
           {
               $this->nmgp_cmp_readonly['num_cita'] = 'on';
           }
?>
...
?>
   ...
   <?php
    if (!isset($this->nm_new_label['num_cita']))
    {
        $this->nm_new_label['num_cita'] = "No. CITA";
    }
?>
<?php
   $nm_cor_fun_cel  = ($nm_cor_fun_cel  == $this->Ini->cor_grid_impar ? $this->Ini->cor_grid_par : $this->Ini->cor_grid_impar);
   $nm_img_fun_cel  = ($nm_img_fun_cel  == $this->Ini->img_fun_imp    ? $this->Ini->img_fun_par  : $this->Ini->img_fun_imp);
   $num_cita = $this->num_cita;
   $sStyleHidden_num_cita = '';
   if (isset($this->nmgp_cmp_hidden['num_cita']) && $this->nmgp_cmp_hidden['num_cita'] == 'off')
   {
       unset($this->nmgp_cmp_hidden['num_cita']);
       $sStyleHidden_num_cita = 'display: none;';
   }
   $bTestReadOnly = true;
   $sStyleReadLab_num_cita = 'display: none;';
   $sStyleReadInp_num_cita = '';
   if (/*($this->nmgp_opcao != "novo" && $this->nmgp_opc_ant != "incluir") || */(isset($this->nmgp_cmp_readonly["num_cita"]) &&  $this->nmgp_cmp_readonly["num_cita"] == "on"))
   {
       $bTestReadOnly = false;
       unset($this->nmgp_cmp_readonly['num_cita']);
       $sStyleReadLab_num_cita = '';
       $sStyleReadInp_num_cita = 'display: none;';
   }
?>
<?php if (isset($this->nmgp_cmp_hidden['num_cita']) && $this->nmgp_cmp_hidden['num_cita'] == 'off') { $sc_hidden_yes++;  ?>
<input type="hidden" name="num_cita" value="<?php echo $this->form_encode_input($num_cita) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOdd css_num_cita_line" id="hidden_field_data_num_cita" style="<?php echo $sStyleHidden_num_cita; ?>vertical-align: top;"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_num_cita_line" style="vertical-align: top;padding: 0px"><span class="scFormLabelOddFormat css_num_cita_label" style=""><span id="id_label_num_cita"><?php echo $this->nm_new_label['num_cita']; ?></span></span><br>
<?php if ($bTestReadOnly && ($this->nmgp_opcao != "novo" && $this->nmgp_opc_ant != "incluir") || (isset($this->nmgp_cmp_readonly["num_cita"]) &&  $this->nmgp_cmp_readonly["num_cita"] == "on")) { 

 ?>
<input type="hidden" name="num_cita" value="<?php echo $this->form_encode_input($num_cita) . "\"><span id=\"id_ajax_label_num_cita\">" . $num_cita . "</span>"; ?>
<?php } else { ?>
<span id="id_read_on_num_cita" class="sc-ui-readonly-num_cita css_num_cita_line" style="<?php echo $sStyleReadLab_num_cita; ?>"><?php echo $this->form_format_readonly("num_cita", $this->form_encode_input($this->num_cita)); ?></span><span id="id_read_off_num_cita" class="css_read_off_num_cita" style="white-space: nowrap;<?php echo $sStyleReadInp_num_cita; ?>">
 <input class="sc-js-input scFormObjectOdd css_num_cita_obj" style="" id="id_sc_field_num_cita" type=text name="num_cita" value="<?php echo $this->form_encode_input($num_cita) ?>"
 size=11 maxlength=11 alt="{datatype: 'integer', maxLength: 11, thousandsSep: '<?php echo str_replace("'", "\'", $this->field_config['num_cita']['symbol_grp']); ?>', thousandsFormat: <?php echo $this->field_config['num_cita']['symbol_fmt']; ?>, allowNegative: false, onlyNegative: false, negativePos: <?php echo (4 == $this->field_config['num_cita']['format_neg'] ? "'suffix'" : "'prefix'") ?>, alignment: 'left', enterTab: false, enterSubmit: false, autoTab: false, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_num_cita_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_num_cita_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>
...
<?php
 if (isset($_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_detalle_factura'] ]) && '' != $_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_detalle_factura'] ]) {
     $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['form_DETALLE_FACTURA_mob_script_case_init'] = $_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_detalle_factura'] ];
 }
 else {
     $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['form_DETALLE_FACTURA_mob_script_case_init'] = $this->Ini->sc_page;
 }
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['form_DETALLE_FACTURA_mob_script_case_init'] ]['form_DETALLE_FACTURA_mob']['embutida_proc']  = false;
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['form_DETALLE_FACTURA_mob_script_case_init'] ]['form_DETALLE_FACTURA_mob']['embutida_form']  = true;
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['form_DETALLE_FACTURA_mob_script_case_init'] ]['form_DETALLE_FACTURA_mob']['embutida_call']  = true;
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['form_DETALLE_FACTURA_mob_script_case_init'] ]['form_DETALLE_FACTURA_mob']['embutida_multi'] = false;
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['form_DETALLE_FACTURA_mob_script_case_init'] ]['form_DETALLE_FACTURA_mob']['embutida_liga_form_insert'] = 'off';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['form_DETALLE_FACTURA_mob_script_case_init'] ]['form_DETALLE_FACTURA_mob']['embutida_liga_form_update'] = 'off';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['form_DETALLE_FACTURA_mob_script_case_init'] ]['form_DETALLE_FACTURA_mob']['embutida_liga_form_delete'] = 'off';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['form_DETALLE_FACTURA_mob_script_case_init'] ]['form_DETALLE_FACTURA_mob']['embutida_liga_form_btn_nav'] = 'off';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['form_DETALLE_FACTURA_mob_script_case_init'] ]['form_DETALLE_FACTURA_mob']['embutida_liga_grid_edit'] = 'on';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['form_DETALLE_FACTURA_mob_script_case_init'] ]['form_DETALLE_FACTURA_mob']['embutida_liga_grid_edit_link'] = 'on';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['form_DETALLE_FACTURA_mob_script_case_init'] ]['form_DETALLE_FACTURA_mob']['embutida_liga_qtd_reg'] = '10';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['form_DETALLE_FACTURA_mob_script_case_init'] ]['form_DETALLE_FACTURA_mob']['embutida_liga_tp_pag'] = 'total';
 ...
 if ($_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['form_DETALLE_FACTURA_mob_script_case_init'] ]['form_FACTURA_mob']['total'] < 0)
 {
 ...
 $sDetailSrc = ('novo' == $this->nmgp_opcao) ? 'form_FACTURA_mob_empty.htm' : $this->Ini->link_form_DETALLE_FACTURA_mob_edit . '?script_case_init=' . $this->form_encode_input($this->Ini->sc_page) . '&script_case_detail=Y';
...
if (isset($this->Ini->sc_lig_target['C_@scinf_detalle_factura']) && 'nmsc_iframe_liga_form_DETALLE_FACTURA_mob' != $this->Ini->sc_lig_target['C_@scinf_detalle_factura'])
{
    if ('novo' != $this->nmgp_opcao)
    {
        $sDetailSrc .= '&under_dashboard=1&dashboard_app=' . $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['dashboard_info']['dashboard_app'] . '&own_widget=' . $this->Ini->sc_lig_target['C_@scinf_detalle_factura'] . '&parent_widget=' . $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['dashboard_info']['own_widget'];
        $sDetailSrc  = $this->addUrlParam($sDetailSrc, 'script_case_init', $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['form_DETALLE_FACTURA_mob_script_case_init']);
    }
?>
<script type="text/javascript">
$(function() {
...
{
?>
<iframe border="0" id="nmsc_iframe_liga_form_DETALLE_FACTURA_mob"  marginWidth="0" marginHeight="0" frameborder="0" valign="top" height="100" width="100%" name="nmsc_iframe_liga_form_DETALLE_FACTURA_mob"  scrolling="auto" src="<?php echo $sDetailSrc; ?>"></iframe>
<?php
}
?>
...
<script>
<?php
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['masterValue']))
{
...
<script>
 var dbParentFrame = $(parent.document).find("[name='<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['dashboard_info']['parent_widget']; ?>']");
 dbParentFrame[0].contentWindow.scAjaxDetailStatus("form_FACTURA_mob");
</script>
<?php
    }
    else {
...
?>
<script>
 parent.scAjaxDetailStatus("form_FACTURA_mob");
 parent.scAjaxDetailHeight("form_FACTURA_mob", <?php echo $sTamanhoIframe; ?>);
</script>
<?php
    }
}
...
?>
<?php
$_SESSION['sc_session'][$this->Ini->sc_page]['form_FACTURA_mob']['buttonStatus'] = $this->nmgp_botoes;
?>
<script type="text/javascript">
