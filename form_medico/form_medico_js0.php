<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="medcodigo" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['medcodigo']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_medico']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
function scForm_packMultiSelect_multi() {
	...
	for (var Nm_x = 1; Nm_x < NM_count_mult; Nm_x++) {
		nm_pack('especialidades' + Nm_x + '_dest', 'especialidades' + Nm_x);
	}
...
function lookup_especialidades(campo)
{
   dados = "";
   nm_pack('especialidades_dest', 'especialidades_1')
   if (campo == 0)
   {
       dados  = document.F1.espcodigo_especialidades.value;
   }
   if (campo == 1)
   {
       dados += "," + document.F1.espnombre_especialidades.value;
   }
   else
   {
       dados += ",";
   }
   dados += "," + document.F1.especialidades_1.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_medico')?>/form_medico.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_especialidades, "especialidades", dados, false);
}
function recebe_especialidades(string)
{
   nm_quant_pack += 100;
   nm_quant_volta = 0;
   nm_del_combo('especialidades_orig');
   if (string != " ")
   {
       ...
       for (var i=0; i < nm_arr_item.length; i++)
       {
            ...
            nm_add_item('especialidades_orig', nm_campos[1], nm_campos[0]);
            nm_quant_volta++;
       }
   }
   nm_sincroniza('especialidades_orig', 'especialidades_dest');
   if (nm_quant_volta > nm_quant_pack)
   {
       alert ("<?php echo html_entity_decode($this->Ini->Nm_lang['lang_errm_tmeo'], ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>");
   }
}
<?php 
