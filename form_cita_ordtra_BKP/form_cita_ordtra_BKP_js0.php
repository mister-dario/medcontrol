...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_cita_ordtra_BKP']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_cita_ordtra_BKP"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
var Crtl_btn_CONTINUAR = false;
function sc_btn_CONTINUAR()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_CONTINUAR(); }, 500);
      return;
    }
    if (Crtl_btn_CONTINUAR) {return;}
    scJs_confirm("<?php echo html_entity_decode('¿Está seguro(a) que desea continuar?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_CONTINUAR_ok, sc_btn_CONTINUAR_cancel)
}
function sc_btn_CONTINUAR_cancel()
{
}
function sc_btn_CONTINUAR_ok()
{
    Crtl_btn_CONTINUAR = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?CONTINUAR?@?";
    document.F1.action = "./";
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_cita_ordtra_BKP']['where_detal']))
  {
?>  
