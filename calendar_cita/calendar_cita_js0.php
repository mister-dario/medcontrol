<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="citid" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['citid']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['calendar_cita']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="calendar_cita"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['calendar_cita']['where_detal']))
  {
?>  
...
?>
    ...
    { 
       ...
       {
           $("#id_sc_field_citrecurrinfo").val(scAjaxGetFieldRecurInfo("citrecurrinfo"));
           scAjaxProcOn();
...
function lookup_fclnumero(campo)
{
   dados  = document.F1.fclnumero.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('calendar_cita')?>/calendar_cita.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_fclnumero, "fclnumero", dados, false);
}
function recebe_fclnumero(string)
{
   document.getElementById('id_lookup_fclnumero').innerHTML = string;
}
<?php 
...
?>
function sc_citrecurr_onchange()
{
...
{
   nm_field_disabled("hora_fin_mostrar=disabled", "");
   nm_field_disabled("fclnumero=disabled", "U");
   //window.frameElement.style = 'height: 500; width: 500;'; 
//top.document.getElementById("iframe_main_menu").contentWindow.document.getElementById("TB_window").style='height: 500; width: 500;'; 
