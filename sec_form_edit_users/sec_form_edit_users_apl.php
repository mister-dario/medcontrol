   ...
   function ini_controle()
   {
      ...
      if (!$run_iframe && isset($_SESSION['scriptcase']['menu_atual']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['embutida_call'] && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['sc_outra_jan']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['sc_outra_jan']))
      {
          ...
          if ($this->Ini->sc_page == $this->sc_init_menu && !isset($_SESSION['scriptcase']['menu_apls'][$_SESSION['scriptcase']['menu_atual']][$this->sc_init_menu]['sec_form_edit_users']))
          {
               ...
               $_SESSION['scriptcase']['menu_apls'][$_SESSION['scriptcase']['menu_atual']][$this->sc_init_menu]['sec_form_edit_users']['label'] = "" . $this->Ini->Nm_lang['lang_othr_frmu_titl'] . " - " . $this->Ini->Nm_lang['lang_list_users'] . "";
      ...
      if ($this->nmgp_opcao == "fast_search")  
      {
          ...
          $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['opcao'] = "inicio";
...
//
   ...
   $nm_nao_carga = false;
   $nmgp_def_dados = "" ; 
   if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_medico']))
   {
       $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_medico'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_medico']); 
   }
   else
   {
       $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_medico'] = array(); 
    }
   ...
   if ($nm_comando != "" && $rs = $this->Db->SelectLimit($nm_comando, 10, 0))
   {
       ...
       { 
              ...
              $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_medico'][] = $rs->fields[0];
              $rs->MoveNext() ; 
       } 
       $rs->Close() ; 
   } 
...
if (isset($this->NM_ajax_flag) && $this->NM_ajax_flag)
{
  ...
  $error_test    = $this->tipo ==2 && (!isset($this->medico ) || empty($this->medico ));    
$error_message = 'Debe seleccionar un médico para asignar a este usuario.'; 
...
$_SESSION['scriptcase']['sec_form_edit_users']['contr_erro'] = 'off'; 
    ...
    function ValidateField_login(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
      ...
      if ($this->nmgp_opcao == "incluir")
      { 
          if (NM_utf8_strlen($this->login) > 22) 
          { 
              $hasError = true;
              $Campos_Crit .= "" . $this->Ini->Nm_lang['lang_sec_users_fild_login'] . " " . $this->Ini->Nm_lang['lang_errm_mxch'] . " 22 " . $this->Ini->Nm_lang['lang_errm_nchr']; 
              if (!isset($Campos_Erros['login']))
              {
                  $Campos_Erros['login'] = array();
              }
              $Campos_Erros['login'][] = $this->Ini->Nm_lang['lang_errm_mxch'] . " 22 " . $this->Ini->Nm_lang['lang_errm_nchr'];
              if (!isset($this->NM_ajax_info['errList']['login']) || !is_array($this->NM_ajax_info['errList']['login']))
              {
                  $this->NM_ajax_info['errList']['login'] = array();
              }
              $this->NM_ajax_info['errList']['login'][] = $this->Ini->Nm_lang['lang_errm_mxch'] . " 22 " . $this->Ini->Nm_lang['lang_errm_nchr'];
          } 
      } 
        if ($hasError) {
            global $sc_seq_vert;
    ...
    function ValidateField_tipo(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
      ...
      { 
        if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['php_cmp_required']['tipo']) || $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['php_cmp_required']['tipo'] == "on")
        { 
          $hasError = true;
          $Campos_Falta[] = "Perfil" ;
    ...
    function ValidateField_medico(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
      ...
      if ($this->nmgp_opcao != "excluir") 
      { 
          if (NM_utf8_strlen($this->medico) > 11) 
          { 
              $hasError = true;
              $Campos_Crit .= "Médico Asignado " . $this->Ini->Nm_lang['lang_errm_mxch'] . " 11 " . $this->Ini->Nm_lang['lang_errm_nchr']; 
              if (!isset($Campos_Erros['medico']))
              {
                  $Campos_Erros['medico'] = array();
              }
              $Campos_Erros['medico'][] = $this->Ini->Nm_lang['lang_errm_mxch'] . " 11 " . $this->Ini->Nm_lang['lang_errm_nchr'];
              ...
              $this->NM_ajax_info['errList']['medico'][] = $this->Ini->Nm_lang['lang_errm_mxch'] . " 11 " . $this->Ini->Nm_lang['lang_errm_nchr'];
          } 
      } 
        if ($hasError) {
            global $sc_seq_vert;
    ...
    function ValidateField_sucursal(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
        global $teste_validade;
        $hasError = false;
      if ($this->sucursal == "" && $this->nmgp_opcao != "excluir")
      { 
      } 
    ...
    function ValidateField_priv_admin(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
      ...
      if ($this->nmgp_opcao != "excluir") 
      { 
          if (NM_utf8_strlen($this->priv_admin) > 255) 
          { 
              $hasError = true;
              $Campos_Crit .= "" . $this->Ini->Nm_lang['lang_sec_users_fild_priv_admin'] . " " . $this->Ini->Nm_lang['lang_errm_mxch'] . " 255 " . $this->Ini->Nm_lang['lang_errm_nchr']; 
              if (!isset($Campos_Erros['priv_admin']))
              {
                  $Campos_Erros['priv_admin'] = array();
              }
              $Campos_Erros['priv_admin'][] = $this->Ini->Nm_lang['lang_errm_mxch'] . " 255 " . $this->Ini->Nm_lang['lang_errm_nchr'];
              ...
              $this->NM_ajax_info['errList']['priv_admin'][] = $this->Ini->Nm_lang['lang_errm_mxch'] . " 255 " . $this->Ini->Nm_lang['lang_errm_nchr'];
          } 
      } 
        if ($hasError) {
            global $sc_seq_vert;
...
$aLookup[] = array(sec_form_edit_users_pack_protect_string('0') => str_replace('<', '&lt;',sec_form_edit_users_pack_protect_string("Super Usuario")));
$aLookup[] = array(sec_form_edit_users_pack_protect_string('1') => str_replace('<', '&lt;',sec_form_edit_users_pack_protect_string("Administrador")));
$aLookup[] = array(sec_form_edit_users_pack_protect_string('2') => str_replace('<', '&lt;',sec_form_edit_users_pack_protect_string("Médico")));
$aLookup[] = array(sec_form_edit_users_pack_protect_string('3') => str_replace('<', '&lt;',sec_form_edit_users_pack_protect_string("Digitador")));
$_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_tipo'][] = '0';
$_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_tipo'][] = '1';
$_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_tipo'][] = '2';
$_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_tipo'][] = '3';
          $aLookupOrig = $aLookup;
          $sSelComp = "name=\"tipo\"";
          ...
          {
              $this->NM_ajax_info['fldList']['tipo']['valList'][$i] = sec_form_edit_users_pack_protect_string($v);
          }
   ...
   $GLOBALS["NM_ERRO_IBASE"] = 0; 
          ...
          $val_output = isset($aLookup[0][sec_form_edit_users_pack_protect_string(NM_charset_to_utf8($this->medico))]) ? $aLookup[0][sec_form_edit_users_pack_protect_string(NM_charset_to_utf8($this->medico))] : "";
...
$aLookup[] = array(sec_form_edit_users_pack_protect_string('Y') => str_replace('<', '&lt;',sec_form_edit_users_pack_protect_string("" . $this->Ini->Nm_lang['lang_opt_yes'] . "")));
$aLookup[] = array(sec_form_edit_users_pack_protect_string('N') => str_replace('<', '&lt;',sec_form_edit_users_pack_protect_string("" . $this->Ini->Nm_lang['lang_opt_no'] . "")));
...
$aLookup[] = array(sec_form_edit_users_pack_protect_string('1') => str_replace('<', '&lt;',sec_form_edit_users_pack_protect_string("Quito")));
$aLookup[] = array(sec_form_edit_users_pack_protect_string('2') => str_replace('<', '&lt;',sec_form_edit_users_pack_protect_string("Guayaquil")));
$_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_sucursal'][] = '1';
$_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['Lookup_sucursal'][] = '2';
          $aLookupOrig = $aLookup;
          $sSelComp = "name=\"sucursal\"";
          if (isset($this->NM_ajax_info['select_html']['sucursal']) && !empty($this->NM_ajax_info['select_html']['sucursal']))
          {
              $sSelComp = $this->NM_ajax_info['select_html']['sucursal'];
          }
          $sLookup = '';
          if (empty($aLookup))
          {
          ...
          {
              $this->NM_ajax_info['fldList']['sucursal']['valList'][$i] = sec_form_edit_users_pack_protect_string($v);
          }
          foreach ($aLookupOrig as $aValData)
          {
...
if($this->tipo ==2) {
	...
	$this->nmgp_cmp_hidden["medico"] = "on"; $this->NM_ajax_info['fieldDisplay']['medico'] = 'on';
	...
	$this->nmgp_cmp_hidden["medico"] = "off"; $this->NM_ajax_info['fieldDisplay']['medico'] = 'off';
...
$this->pswd  = $this->rs[0][0];
$this->confirm_pswd  = $this->rs[0][0];
...
$_SESSION['scriptcase']['sec_form_edit_users']['contr_erro'] = 'off'; 
      ...
      if ($this->tipo === "" || is_null($this->tipo))  
      { 
          $this->tipo = 0;
          $this->sc_force_zero[] = 'tipo';
      } 
      if ($this->nmgp_opcao == "alterar")
      {
      if ($this->medico === "" || is_null($this->medico))  
      { 
          $this->medico = 0;
          $this->sc_force_zero[] = 'medico';
      } 
      }
      ...
      if ($this->nmgp_opcao == "alterar") 
      {
          ...
          { 
              ...
              { 
                  ...
                  $SC_fields_update[] = "name = '$this->name', email = '$this->email', active = '$this->active', priv_admin = '$this->priv_admin', SUCURSAL = $this->sucursal, TIPO = $this->tipo, medico = $this->medico"; 
              } 
              elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
              { 
                  $comando = "UPDATE " . $this->Ini->nm_tabela . " SET ";  
             ...
             { 
              ...
              $this->nmgp_refresh_fields = array_diff(array('login', 'pswd', 'confirm_pswd', 'name', 'email', 'tipo', 'medico', 'active', 'sucursal', 'priv_admin'), $aDoNotUpdate);
              $this->ajax_return_values();
      ...
      { 
          ...
          { 
              ...
              { 
                  ...
                  { 
                       $compl_insert     .= ", medico";
                       $compl_insert_val .= ", $this->medico";
                  } 
                  $comando = "INSERT INTO " . $this->Ini->nm_tabela . " (login, pswd, name, email, active, activation_code, priv_admin, SUCURSAL, TIPO $compl_insert) VALUES ('$this->login', '$this->pswd', '$this->name', '$this->email', '$this->active', '$this->activation_code', '$this->priv_admin', $this->sucursal, $this->tipo $compl_insert_val)"; 
              }
              elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
              { 
                  $compl_insert     = ""; 
                  ...
                  $comando = "INSERT INTO " . $this->Ini->nm_tabela . " (" . $NM_cmp_auto . "login, pswd, name, email, active, activation_code, priv_admin, SUCURSAL, TIPO $compl_insert) VALUES (" . $NM_seq_auto . "'$this->login', '$this->pswd', '$this->name', '$this->email', '$this->active', '$this->activation_code', '$this->priv_admin', $this->sucursal, $this->tipo $compl_insert_val)"; 
              }
...
//------------ 
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['run_iframe'] == "F" || $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['run_iframe'] == "R")
      {
          if ($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['iframe_evento'] == "insert") 
          { 
               $this->nmgp_opcao = "novo"; 
      ...
      { 
          ...
          if ($this->nmgp_opcao == "igual" && isset($this->NM_btn_navega) && 'S' == $this->NM_btn_navega && !empty($this->login))
          {
              ...
              $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['reg_start'] = $rt->fields[0];
              $rt->Close(); 
          }
      } 
      else 
      { 
          $qt_geral_reg_sec_form_edit_users = $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['total'];
      } 
      ...
      { 
          $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['reg_start']++; 
          if ($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['reg_start'] > $qt_geral_reg_sec_form_edit_users)
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['reg_start'] = $qt_geral_reg_sec_form_edit_users; 
          }
      } 
      ...
      if ($this->nmgp_opcao == "navpage" && ($this->nmgp_ordem - 1) <= $qt_geral_reg_sec_form_edit_users) 
      { 
          $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['reg_start'] = $this->nmgp_ordem - 1; 
      } 
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['reg_start']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['reg_start']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['reg_start'] = 0;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['reg_qtd'] = $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['reg_start'] + 1;
      $this->NM_ajax_info['navSummary']['reg_ini'] = $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['reg_start'] + 1; 
      $this->NM_ajax_info['navSummary']['reg_qtd'] = $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['reg_qtd']; 
      $this->NM_ajax_info['navSummary']['reg_tot'] = $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['total'] + 1; 
...
//---------- 
      ...
      { 
          ...
          if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
          { 
              $nmgp_select = "SELECT login, pswd, name, email, active, activation_code, priv_admin, SUCURSAL, TIPO, medico from " . $this->Ini->nm_tabela ; 
          } 
          elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
          { 
              $nmgp_select = "SELECT login, pswd, name, email, active, activation_code, priv_admin, SUCURSAL, TIPO, medico from " . $this->Ini->nm_tabela ; 
...
//-- 
   ...
   function NM_gera_nav_page() 
   {
       ...
       $rec_tot    = $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['total'] + 1;
       $rec_fim    = $_SESSION['sc_session'][$this->Ini->sc_page]['sec_form_edit_users']['reg_start'] + 1;
...
$modificado_medico = $this->medico;
$this->nm_formatar_campos('tipo', 'medico');
...
if ($original_medico !== $modificado_medico || isset($this->nmgp_cmp_readonly['medico']) || (isset($bFlagRead_medico) && $bFlagRead_medico))
{
    $this->ajax_return_values_medico(true);
}
...
<?php
    ...
    {
        $searchOk = false;
        if ('SC_all_Cmp' == $this->nmgp_fast_search && in_array($field, array("login", "pswd", "name", "email", "active", "activation_code", "priv_admin", "sucursal", "clave_token"))) {
            $searchOk = true;
        }
...
function sc_file_size($file, $format = false)
{
 ...
 function new_date_format($type, $field)
 {
   ...
   {
       $nmgp_def_dados  = "";
       $nmgp_def_dados .= "Super Usuario?#?0?#?N?@?";
       $nmgp_def_dados .= "Administrador?#?1?#?N?@?";
       $nmgp_def_dados .= "Médico?#?2?#?N?@?";
       $nmgp_def_dados .= "Digitador?#?3?#?N?@?";
   ...
   {
       $nmgp_def_dados  = "";
       $nmgp_def_dados .= "Quito?#?1?#??@?";
       $nmgp_def_dados .= "Guayaquil?#?2?#??@?";
       $todo = explode("?@?", $nmgp_def_dados);
       return $todo;
   ...
   function SC_fast_search($in_fields, $arg_search, $data_search)
   {
      ...
      foreach ($fields as $field) {
          ...
          if ($field == "SC_all_Cmp") 
          {
              $data_lookup = $this->SC_lookup_sucursal($arg_search, $data_search);
              if (is_array($data_lookup) && !empty($data_lookup)) 
              {
                  $this->SC_monta_condicao($comando, "SUCURSAL", $arg_search, $data_lookup);
              }
          }
      }
   ...
   function SC_monta_condicao(&$comando, $nome, $condicao, $campo, $tp_campo="")
   {
      ...
      $nm_ini_lower = "";
      $nm_fim_lower = "";
      $nm_numeric[] = "sucursal";$nm_numeric[] = "tipo";$nm_numeric[] = "medico";
   ...
   function SC_lookup_sucursal($condicao, $campo)
   {
       $data_look = array();
       $campo  = substr($this->Db->qstr($campo), 1, -1);
       $data_look['1'] = "Quito";
       $data_look['2'] = "Guayaquil";
       $result = array();
       foreach ($data_look as $chave => $label) 
       {
           if ($condicao == "eq" && $campo == $label)
