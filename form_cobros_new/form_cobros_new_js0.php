...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_cobros_new']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_cobros_new"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_cobros_new']['where_detal']))
  {
?>  
...
?>
    ...
    if (x == "alterar") 
    { 
       if (confirm ("¿Está seguro(a) que desea continuar?"))  
       { }
       else 
       { 
          return; 
...
function scForm_confirmUpdate_single(callbackOk, callbackCancel) {
	scJs_confirm("¿Está seguro(a) que desea continuar?", callbackOk, callbackCancel);
