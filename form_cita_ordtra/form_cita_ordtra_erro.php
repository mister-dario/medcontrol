...
$old_error_handler = set_error_handler("form_cita_ordtra_scriptcase_error_handler");

function form_cita_ordtra_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_cita_ordtra']['contr_erro']) && $_SESSION['scriptcase']['form_cita_ordtra']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_cita_ordtra->contr_form_cita_ordtra->Apl_com_erro = true;
            if ($inicial_form_cita_ordtra->contr_form_cita_ordtra->NM_ajax_flag)
            {
                $inicial_form_cita_ordtra->contr_form_cita_ordtra->NM_ajax_info['errList']['geral_form_cita_ordtra'][] = $msg;
                return;
            }
            form_cita_ordtra_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_cita_ordtra_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_cita_ordtra'])  && '' != $_SESSION['scriptcase']['error_icon']['form_cita_ordtra'])  ? $_SESSION['scriptcase']['error_icon']['form_cita_ordtra']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_cita_ordtra']) && '' != $_SESSION['scriptcase']['error_close']['form_cita_ordtra']) ? $_SESSION['scriptcase']['error_close']['form_cita_ordtra'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_cita_ordtra']) && '' != $_SESSION['scriptcase']['error_span_title']['form_cita_ordtra']) ? $_SESSION['scriptcase']['error_span_title']['form_cita_ordtra'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_cita_ordtra']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_cita_ordtra']) ? $_SESSION['scriptcase']['error_icon_title']['form_cita_ordtra'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_cita_ordtra']['error_buffer'] .= ob_get_flush();
}
